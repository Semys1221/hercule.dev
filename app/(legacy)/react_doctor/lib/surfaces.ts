...
export function classifySurface(relPath: string): Surface {
  ...
  if (
    rel.startsWith("app/(legacy)/internal/") ||
    rel.startsWith("components/legacy/internal/")
  ) {
    return "internal";
  }
  if (
    rel.startsWith("components/site/") ||
    rel.startsWith("components/legacy/agence/") ||
    rel === "app/(marketing)/page.tsx" ||
    rel.startsWith("app/(marketing)/")
  ) {
    return "marketing";
  }
  if (
    rel.startsWith("app/(legacy)/dashboard/") ||
    rel.startsWith("components/legacy/dashboard/")
  ) {
    return "dashboard";
  }
