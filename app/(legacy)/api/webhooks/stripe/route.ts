...
} from "@/lib/legacy/payments/stripe-webhook-comptable";
import {
...
import { handleComptableAcquisitionCheckoutCompleted } from "@/lib/legacy/payments/stripe-webhook-comptable-acquisition";
import {
  handleHerculeLiberalCheckoutCompleted,
  isHerculeLiberalCheckoutSession,
} from "@/lib/legacy/payments/stripe-webhook-hercule-liberal";
import {
  handleSaasAutonomeCheckoutCompleted,
  isSaasAutonomeCheckoutSession,
} from "@/lib/legacy/payments/stripe-webhook-saas-autonome";
import {
  insertJob,
  markJobFailed,
  markJobSent,
    ...
    if (agenceLead) {
      ...
      try {
        const { sendProductEmailNow } = await import(
          "@/lib/legacy/booking-communication/product-send"
        );
