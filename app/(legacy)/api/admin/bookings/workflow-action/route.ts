...
import { upsertIdsForLeadCategory } from "@/lib/legacy/sales-calls/resolve-lead";
import type { SalesCall, SalesCallStatus } from "@/lib/legacy/sales-calls/types";
...
async function startSequenceForStatus(
  ...
  if (status === "not_paid") {
    const { startCloseIndecisSequence } = await import(
      "@/lib/legacy/close-indecis-sequence/orchestrator"
    );
    return startCloseIndecisSequence(salesCall, leadId, category);
  }

  const { startNoShowSequence } = await import("@/lib/legacy/no-show-sequence/orchestrator");
