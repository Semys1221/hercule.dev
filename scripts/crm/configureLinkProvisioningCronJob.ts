/**
 * Register (or update) the cron-job.org job for link-provisioning.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule link-provisioning";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/link-provisioning`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/link-provisioning"),
    ) ?? null
  );
}
...
async function main(): Promise<void> {
  ...
  if (existing?.jobId) {
    ...
    console.log(
      `Updated cron-job.org job ${existing.jobId} → ${payload.job.url} (hourly)`,
    );
    return;
  }
  ...
  console.log(
    `Created cron-job.org job ${created.jobId ?? "?"} → ${payload.job.url} (hourly)`,
  );
}
