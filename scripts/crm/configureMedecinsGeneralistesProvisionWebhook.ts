/**
 * Register Instantly email_sent webhook → provision JUM links for médecins généralistes.
 *
 * Usage:
 *   pnpm configure-medecins-generalistes-provision-webhook
 */
import {
  createWebhook,
  listWebhooks,
...
const MEDECIN_CAMPAIGN_ID =
  process.env.MEDECINS_GENERALISTES_CAMPAIGN_ID?.trim() || vertical.campaignId;

const WEBHOOK_NAME = "Hercule JUM medecin provision (email_sent)";
...
function publicWebhookUrl(): string {
  ...
  if (appUrl && appUrl.startsWith("https://") && !appUrl.includes("localhost")) {
    return `${appUrl}/api/webhooks/instantly/provision-jum-medecin`;
  }
  return "https://www.hercule.dev/api/webhooks/instantly/provision-jum-medecin";
}
...
function matchesMedecinProvisionWebhook(
...
async function main(): Promise<void> {
  ...
  const match = existing.find((hook) =>
    matchesMedecinProvisionWebhook(hook, targetUrl),
  );
