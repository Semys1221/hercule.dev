/**
 * Register Instantly email_sent webhook → provision JUM links for kinésithérapeutes.
 *
 * Usage:
 *   pnpm configure-jum-kine-provision-webhook
 */
import {
  createWebhook,
  listWebhooks,
...
const WEBHOOK_NAME = "Hercule JUM kine provision (email_sent)";
...
function publicWebhookUrl(): string {
  ...
  if (appUrl && appUrl.startsWith("https://") && !appUrl.includes("localhost")) {
    return `${appUrl}/api/webhooks/instantly/provision-jum-kine`;
  }
  return "https://www.hercule.dev/api/webhooks/instantly/provision-jum-kine";
}
