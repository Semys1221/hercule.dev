/**
 * Register Instantly email_sent webhook → provision JUM links for avocats.
 *
 * Usage:
 *   pnpm configure-jum-avocat-provision-webhook
 */
import {
  createWebhook,
  listWebhooks,
...
const WEBHOOK_NAME = "Hercule JUM avocat provision (email_sent)";
...
function publicWebhookUrl(): string {
  ...
  if (appUrl && appUrl.startsWith("https://") && !appUrl.includes("localhost")) {
    return `${appUrl}/api/webhooks/instantly/provision-jum-avocat`;
  }
  return "https://www.hercule.dev/api/webhooks/instantly/provision-jum-avocat";
}
