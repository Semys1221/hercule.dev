/**
 * Register (or update) the cron-job.org job for post-rdv-surveys.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule post-rdv-surveys";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/post-rdv-surveys`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/post-rdv-surveys"),
    ) ?? null
  );
}
