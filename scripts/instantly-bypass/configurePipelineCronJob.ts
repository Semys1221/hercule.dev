/**
 * Register (or update) the cron-job.org job for instantly-bypass-pipeline.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule instantly-bypass-pipeline";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/instantly-bypass-pipeline`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/instantly-bypass-pipeline"),
    ) ?? null
  );
}
