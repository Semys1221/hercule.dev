/**
 * Register (or update) the cron-job.org job for instantly-bypass-jobs.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule instantly-bypass-jobs";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/instantly-bypass-jobs`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/instantly-bypass-jobs"),
    ) ?? null
  );
}
...
async function main(): Promise<void> {
  ...
  if (existing?.jobId) {
    ...
    console.log(
      `Updated cron-job.org job ${existing.jobId} → ${payload.job.url} (every 10 min)`,
    );
    return;
  }
  ...
  console.log(
    `Created cron-job.org job ${created.jobId ?? "?"} → ${payload.job.url} (every 10 min)`,
  );
}
