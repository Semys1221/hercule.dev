...
import type { DemandeContrat, DemandeStatus, DemandeTeaser } from "@/lib/site/demandes-data"
import { Card } from "@/components/ui/card"

function DemandeStatusBadge({ status }: { status: DemandeStatus }) {
  if (status === "assigned") {
    return (
      <span className="inline-flex items-center gap-1 rounded-full border border-border/80 bg-muted/90 px-2.5 py-0.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground shrink-0">
  ...
  return (
    <div
      className={cn(
        ...
        !isAssigned &&
          "hover:[&_.demande-flip-shell]:border-zinc-600 hover:[&_.demande-flip-shell]:bg-muted/80 hover:[&_.demande-flip-shell]:ring-1 hover:[&_.demande-flip-shell]:ring-white/10",
    ...
    >
      <DemandeFlipCard
        variant="marketing"
        disabled={isAssigned}
        versoCriteria={CIF_DEMANDE_VERSO_CRITERIA}
        className="demande-flip-shell border-border bg-muted/50"
        versoFields={{
        ...
        recto={
          <>
            <CardHeader className="relative gap-2 border-b border-border px-5 py-4 pr-12">
              ...
              {demande.origine ? (
                <p className="text-foreground/90 text-xs font-medium">{demande.origine}</p>
              ) : null}
            </CardHeader>

            <CardContent className="flex flex-1 flex-col gap-4 px-5 py-4">
              <p className="text-foreground text-sm font-medium leading-snug">{demande.prestation}</p>
...
export function CarteTeaser({ teaser }: { teaser: DemandeTeaser }) {
  ...
  return (
    <Card className="h-full min-h-[260px] gap-0 rounded-2xl border-dashed border-border bg-muted/30 py-0 shadow-none">
      <CardHeader className="gap-2 border-b border-dashed border-border px-5 py-4">
        ...
        <p className="text-muted-foreground text-xs">En cours de qualification · Bientôt</p>
      </CardHeader>

      <CardContent className="flex flex-1 flex-col gap-3 px-5 py-4">
        <p className="text-muted-foreground text-sm leading-relaxed">{teaser.description}</p>
        <p className="text-muted-foreground text-xs leading-relaxed mt-auto">{teaser.note}</p>
      </CardContent>
    </Card>
  )
}
