/**
 * Register (or update) the cron-job.org job for ai-reply-agent-jobs.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule ai-reply-agent-jobs";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/ai-reply-agent-jobs`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/ai-reply-agent-jobs"),
    ) ?? null
  );
}
