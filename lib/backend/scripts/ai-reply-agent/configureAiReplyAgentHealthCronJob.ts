/**
 * Register (or update) the cron-job.org job for ai-reply-agent-health.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule ai-reply-agent-health";
const SCHEDULE_MINUTES = [5, 35];
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/ai-reply-agent-health`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/ai-reply-agent-health"),
    ) ?? null
  );
}
...
async function main(): Promise<void> {
  ...
  if (existing?.jobId) {
    ...
    console.log(
      `Updated cron-job.org job ${existing.jobId} → ${payload.job.url} (every 30 min)`,
    );
    return;
  }
  ...
  console.log(
    `Created cron-job.org job ${created.jobId ?? "?"} → ${payload.job.url} (every 30 min)`,
  );
}
