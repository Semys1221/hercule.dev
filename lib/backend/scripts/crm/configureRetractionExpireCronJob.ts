/**
 * Register (or update) the cron-job.org job for retraction-expire.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule retraction-expire";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/retraction-expire`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/retraction-expire"),
    ) ?? null
  );
}
...
async function main(): Promise<void> {
  ...
  if (existing?.jobId) {
    ...
    console.log(
      `Updated cron-job.org job ${existing.jobId} → ${payload.job.url} (hourly :15 Paris)`,
    );
    return;
  }
  ...
  console.log(
    `Created cron-job.org job ${created.jobId ?? "?"} → ${payload.job.url} (hourly :15 Paris)`,
  );
}
