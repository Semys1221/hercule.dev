...
const MIGRATION_FILE = path.join(
  ...
  "../../supabase/migrations/20260931120000_agence_demandes_qualification_verso.sql",
);
...
async function main(): Promise<void> {
  ...
  console.log(
    `Applying ${path.basename(MIGRATION_FILE)} to ${projectRefFromEnv()}...`,
  );
  try {
    ...
    if (/already exists|duplicate column|duplicate key/i.test(message)) {
