/**
 * Register (or update) the cron-job.org job for management-recipients-sync.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule management-recipients-sync";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/management-recipients-sync`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/management-recipients-sync"),
    ) ?? null
  );
}
...
async function main(): Promise<void> {
  ...
  if (existing?.jobId) {
    ...
    console.log(
      `Updated cron-job.org job ${existing.jobId} → ${payload.job.url} (hourly :30 Paris)`,
    );
    return;
  }
  ...
  console.log(
    `Created cron-job.org job ${created.jobId ?? "?"} → ${payload.job.url} (hourly :30 Paris)`,
  );
}
