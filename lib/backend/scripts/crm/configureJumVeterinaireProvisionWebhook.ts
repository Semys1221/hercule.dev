/**
 * Register Instantly email_sent webhook → provision JUM links for vétérinaires.
 *
 * Usage:
 *   pnpm configure-jum-veterinaire-provision-webhook
 */
import {
  createWebhook,
  listWebhooks,
...
const vertical = getJumVertical("veterinaire");
const CAMPAIGN_ID =
  process.env.VETERINAIRES_CAMPAIGN_ID?.trim() || vertical.campaignId;

const WEBHOOK_NAME = "Hercule JUM veterinaire provision (email_sent)";
...
function publicWebhookUrl(): string {
  ...
  if (appUrl && appUrl.startsWith("https://") && !appUrl.includes("localhost")) {
    return `${appUrl}/api/webhooks/instantly/provision-jum-veterinaire`;
  }
  return "https://www.hercule.dev/api/webhooks/instantly/provision-jum-veterinaire";
}
