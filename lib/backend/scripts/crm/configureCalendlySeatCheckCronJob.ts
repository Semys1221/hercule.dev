/**
 * Register (or update) the cron-job.org job for calendly-seat-check.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule calendly-seat-check";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/calendly-seat-check`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/calendly-seat-check"),
    ) ?? null
  );
}
