...
const ENUM_QUERY = `
ALTER TYPE public.lead_statut ADD VALUE IF NOT EXISTS 'MEETING_BOOKED';
ALTER TYPE public.lead_statut ADD VALUE IF NOT EXISTS 'CONFIRMED';
`;
...
async function main(): Promise<void> {
  const projectRef = projectRefFromEnv();
  console.log(`Adding enum values on ${projectRef}...`);
  try {
    await applyViaManagementApi(ENUM_QUERY);
  } catch (err) {
    if (!isBenignAlreadyExists(err)) {
      throw err;
    }
    console.log("Enum values already present.");
  }
