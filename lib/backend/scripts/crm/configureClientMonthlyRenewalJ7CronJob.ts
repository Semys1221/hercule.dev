/**
 * Register (or update) the daily cron-job.org job for the J-7 renewal email.
 *
 * Requires in .env:
 *   CRON_JOB_ORG_API_KEY
 *   CRON_SECRET
 *
 * Schedule: 08:00 Europe/Paris, every day.
 */

const CRON_JOB_API = "https://api.cron-job.org";
const JOB_TITLE = "hercule client-monthly-renewal-j7";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/client-monthly-renewal-j7`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/client-monthly-renewal-j7"),
    ) ?? null
  );
}
