/**
 * Register (or update) the cron-job.org job for Gmail client inbox sync.
 *
 * Requires in .env:
...
const JOB_TITLE = "hercule gmail-client-inbox";
...
function cronTargetUrl(): string {
  ...
  return `${base}/api/cron/gmail-client-inbox`;
}
  ...
  return (
    page.jobs?.find(
      (job) =>
        job.url === target ||
        job.title === JOB_TITLE ||
        job.url?.includes("/api/cron/gmail-client-inbox"),
    ) ?? null
  );
}
...
async function main(): Promise<void> {
  ...
  if (existing?.jobId) {
    ...
    console.log(
      `Updated cron-job.org job ${existing.jobId} → ${payload.job.url} (every 5 min)`,
    );
    return;
  }
  ...
  console.log(
    `Created cron-job.org job ${created.jobId ?? "?"} → ${payload.job.url} (every 5 min)`,
  );
}
