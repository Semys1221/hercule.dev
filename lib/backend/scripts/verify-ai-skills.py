...
SPOT_CHECKS = [
    {
        "id": "nextjs-internal",
        "path": "app/(legacy)/internal/(app-shell)/clients/page.tsx",
    ...
    },
    {
        ...
        "path": "lib/backend/streamlit_scraper/core_logic.py",
...
STREAMLIT_READMES = list((ROOT / "app" / "(legacy)").glob("streamlit_*/README.md"))
NEXTJS_DOC_POINTERS = [
    ROOT / "README.md",
    ROOT / "doc/README.md",
    ROOT / "app/(legacy)/internal/README.md",
    ROOT / "app/(legacy)/crm/doc.md",
    ROOT / "app/(legacy)/dashboard/README.md",
...
def matches_streamlit_rule(path: str) -> bool:
    ...
    return normalized.startswith("lib/backend/streamlit_")


def matches_nextjs_rule(path: str) -> bool:
    normalized = path.replace("\\", "/")
    if normalized.startswith("lib/backend/streamlit_"):
        return False
...
def check_rules(errors: list[str]) -> None:
    ...
    if not streamlit_rule.is_file():
        ...
        if "lib/backend/streamlit_" not in text and "lib/backend/streamlit_" not in text:
