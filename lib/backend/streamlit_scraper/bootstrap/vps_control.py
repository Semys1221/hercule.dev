...
def _start_local_worker(preset: str) -> tuple[bool, str]:
    scraper_dir = os.path.join(_REPO_ROOT, "lib", "backend", "streamlit_scraper")
    log_path = os.path.join(_REPO_ROOT, "lib", "backend", "streamlit_scraper", "output", preset, "worker.log")
    os.makedirs(os.path.dirname(log_path), exist_ok=True)
    env = os.environ.copy()
    env["PYTHONPATH"] = os.path.pathsep.join(
        [os.path.join(_REPO_ROOT, "backend"), _REPO_ROOT]
    )
