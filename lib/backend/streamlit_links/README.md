...
# or:
cd lib/backend/streamlit_links && pip install -r requirements.txt && streamlit run app.py
```

Needs the Next.js app for status/sequence APIs (`CRM_BACKEND_URL` or `NEXT_PUBLIC_APP_URL`, default `http://localhost:3000`).

For Calendly bookings + Resend sequences only, use the dedicated tool: `pnpm streamlit-booking-resend` ([lib/backend/streamlit_booking_resend](../streamlit_booking_resend/README.md)).
