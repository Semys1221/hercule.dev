...
import { ensureCampaignLeadLinks } from "@/lib/legacy/link-tracking/provision-campaign-lead";
...
export async function executeBypassFlow(
  ...
  const vars = {
    ...
    ...(await resolveSlotVariables(campaignId, bodyHtml)),
  };
  ...
  if (flow === "interested_email1") {
    const { supersedePreE1Inbounds } = await import(
      "@/lib/legacy/ai-reply-agent/supersede-pre-e1"
    );
