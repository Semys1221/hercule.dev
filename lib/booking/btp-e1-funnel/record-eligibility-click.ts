...
export async function recordBtpE1EligibilityClick(): Promise<void> {
  const client = createBypassClient();
  const { error } = await client.from("btp_e1_eligibility_clicks").insert({});

  if (error) {
    if (isMissingRelationError(error)) {
      throw new Error(
        "btp_e1_eligibility_clicks table is missing — apply Supabase migration",
      );
    }
