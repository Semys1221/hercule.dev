...
function pickEarliest(items: InstantlyEmailRecord[]): InstantlyEmailRecord | null {
  if (items.length === 0) return null;
  return [...items].sort((a, b) => {
    const ta = a.timestamp_email ?? a.timestamp_created ?? "";
    const tb = b.timestamp_email ?? b.timestamp_created ?? "";
...
async function resolveInitialEaccount(
  apiKey: string,
  params: { leadEmail: string; campaignId: string; fallbackEaccount?: string },
): Promise<string | null> {
  ...
  const earliest = pickEarliest(sent);
  if (earliest?.eaccount?.trim()) {
    return earliest.eaccount.trim();
  }
  if (params.fallbackEaccount?.trim()) {
    return params.fallbackEaccount.trim();
  }
  return null;
}

async function resolveReplyAnchor(
  apiKey: string,
  params: { leadEmail: string; campaignId: string },
): Promise<{ replyToUuid: string; subject?: string } | null> {
  const attempts = 3;
...
export async function resolveThreadForReply(
  ...
  const reply = await resolveReplyAnchor(apiKey, {
  ...
  const eaccount = await resolveInitialEaccount(apiKey, {
    leadEmail: params.leadEmail,
    campaignId: params.campaignId,
    fallbackEaccount: params.fallbackEaccount,
  });
  if (!eaccount) {
    return null;
  }
